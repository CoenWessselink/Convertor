"""Representative canonical W18 fixture with non-empty BOM families.
...
round-trip and remain actionable in the BOM read model.
"""
...
from cws_convertor.bom.production_hub import ACTION_DEFINITIONS, BOMActionMatrix, BOMStockAllocator
...
def run():
    ...
    part_row=next(row for row in model.family_rows('parts') if 'P1' in row.entity_ids)
    plan=BOMStockAllocator().plan(project,(part_row,),kerf_mm=3.0,preference='remnants_first')
    ...
    assert snapshot.snapshot_sha256==_snapshot.snapshot_sha256
