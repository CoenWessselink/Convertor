...
class ReviewWorkflowTests(unittest.TestCase):
    ...
    def test_geometry_confirmation_alone_cannot_release_unreviewed_material(self) -> None:
        analysis = analyze_external_pdf(self.source)
        reviewed = apply_review(analysis, build_review_payload(
            analysis.part, reviewed_by="Review Tester", confirm=["holes[0]"]
        ))
        self.assertFalse(reviewed.production_export_allowed)
