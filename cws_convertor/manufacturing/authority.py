"""Unified facade for the frozen Scribing M9-M18 authority layer.

M1-M8 stay canonical in the current Viewer/Convertor manufacturing package.
The frozen M9-M18 runtime is carried by the historic text payload.  Its exact
repository payload fingerprint is checked first.  Connector transport garbage
outside the Base64 alphabet is then removed; after that the decoded ZIP must
still match the frozen M18 runtime SHA-256, pass ZIP CRC/path validation and
report the original source commit before a module can be imported.

Filtering transport garbage therefore cannot make damaged authority code pass:
missing/substituted Base64 data changes the decoded SHA and remains blocked.
"""
...
def _verified_runtime_bytes() -> bytes:
    ...
    for path in _payload_files():
        try:
            normalized.append(b"".join(path.read_bytes().split()))
    ...
    if payload_sha != M18_RUNTIME_PAYLOAD_SHA256:
        raise RuntimeError(
            ...
            f"expected={M18_RUNTIME_PAYLOAD_SHA256} actual={payload_sha}"
        )

    # Some historic GitHub connector writes inserted non-Base64 transport
    # characters into the otherwise checksum-bound text payload.  Remove only
    # those impossible alphabet bytes.  The decoded authority identity below
    # remains the decisive immutable gate.
    clean_payload = bytes(value for value in payload if value in _BASE64_BYTES)
    removed = len(payload) - len(clean_payload)
    if removed <= 0:
        clean_payload = payload
    try:
        runtime = base64.b64decode(clean_payload, validate=True)
    except Exception as exc:
        raise RuntimeError(
            f"Frozen M18 Base64 payload blijft niet strict decodeerbaar na transportreiniging (removed={removed})"
        ) from exc

    runtime_sha = hashlib.sha256(runtime).hexdigest()
    if runtime_sha != M18_RUNTIME_SHA256:
        raise RuntimeError(
            "Frozen M18 decoded runtime SHA-256 wijkt af na transportreiniging; "
            f"expected={M18_RUNTIME_SHA256} actual={runtime_sha} removed={removed}"
        )
...
def _prepare_runtime_import_root() -> Path:
    ...
    try:
        with zipfile.ZipFile(BytesIO(runtime), "r") as source:
            validated = [(info, _validate_runtime_member(info)) for info in source.infolist()]
