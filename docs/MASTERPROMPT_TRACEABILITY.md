...
| B - Viewer & Import Accuracy | Correct repeatable import; source/model/mesh trace; production viewer; measurements; sections; compare; debug mode; visual, golden and large-model regression | Semantic IFC/STEP import, source locators, exact single-solid STEP selection, entity-specific IFC tessellation, SteelModel-bound host UI, hash-bound STEP/IFC/canonical mesh resources, built-in VTK/Tk rendering, synchronized picking/selection, structural visual golden, generated load regression and bounded progressive whole-project loading | **In progress**: batches 1-3 are tested; measurements, sections, compare and owner-validated large-model acceptance remain open |
| C - Production Editor | Viewer-integrated parametric production editing, feature preview, scribing, audit and deterministic rebuild | Part Workbench 1.1, typed feature contract 1.0, production-property editing, exact plate holes/slots/rectangular cutouts, marking intent, strict exact-contact proposals, integrated 2D/3D preview, undo/redo, audit and hash invalidation | **In progress**: batch 1 is locally tested; native VTK placement, automatic contact extraction, complex profile/end/bevel geometry, owner acceptance and fresh Windows package evidence remain open |
