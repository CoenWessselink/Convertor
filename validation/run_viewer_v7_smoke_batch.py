#!/usr/bin/env python3
"""Run Viewer V7 smoke scripts as isolated subprocesses and persist evidence.

The batch range makes the runner usable in constrained CI/analysis shells while
keeping each smoke test independent. A final aggregation call creates the
...
OUT = ROOT / "validation" / "viewer_v7_full_smokes"
LOGS = OUT / "logs"
...
def aggregate() -> dict:
    ...
    summary = {
        "schema": "cws-viewer-v7-full-smoke-summary-1.0",
        "python": sys.version,
    ...
    (OUT / "VIEWER_V7_FULL_SMOKE_SUMMARY.json").write_text(
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional evidence directory outside the repository validation tree.",
    )
