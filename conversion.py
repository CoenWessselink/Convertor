...
def _write_profile_nc1(
    ...
    *,
    ...
    material = core._ascii_safe(material, "")
    ...
    area_mm2 = float(profile.area_mm2 or 0.0)
    if not math.isfinite(area_mm2) or area_mm2 <= 0.0:
        raise ValueError("PROFILE_AREA_REVIEW_REQUIRED: profieloppervlakte voor massa ontbreekt")
    mass = area_mm2 * density_kg_m3 / 1_000_000.0
    lines = [
        "ST",
        f"** Generated by {APP_NAME} v{__version__} from {source_name}",
        f"  {order_number}",
        f"  {part_number}",
        "  1",
        f"  {part_number}",
...
def step_to_nc1(
    ...
    *,
    ...
    """Validate in same-filesystem staging, then atomically replace the target.

    Existing output and the source are untouched if recognition, material
    checks or roundtrip validation fail. The staging artifact keeps the final
    basename so embedded attachment names stay deterministic.
    """
    ...
    if source.resolve() == target.resolve() or (
        ...
        raise ValueError("SOURCE_EQUALS_TARGET: STEP-bron mag niet door NC1-uitvoer worden overschreven")
    target.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix=".cws-nc1-stage-", dir=target.parent) as folder:
        staged_target = Path(folder) / target.name
        result = _step_to_nc1_staged(
            source, staged_target, material=material, order_number=order_number,
            profile_database=profile_database, preferred_profile=preferred_profile,
            tolerance_mm=tolerance_mm, strict_validation=strict_validation,
            embed_converter_payload=embed_converter_payload,
        )
        if not staged_target.is_file():
            raise RuntimeError("NC1_STAGING_ARTIFACT_MISSING: gevalideerde uitvoer ontbreekt")
        os.replace(staged_target, target)
        result.output = target
        return result


def _step_to_nc1_staged(
    input_path: str | Path,
    output_path: str | Path,
    *,
    material: str = "",
    ...
    declared = str(payload.material or "") if payload is not None else ""
    material = resolve_conversion_material(material, source_material=declared, required=True)
    target.parent.mkdir(parents=True, exist_ok=True)
    ...
    if payload is not None:
        ...
        if nc1_bytes:
            ...
            try:
                ...
                embedded_material = resolve_conversion_material(
                    source_material=reconstructed_part.header.material, required=True,
                )
                if embedded_material != material:
                    raise ValueError("MATERIAL_CONFLICT: embedded NC1 wijkt af van canoniek materiaalbewijs")
            ...
            if requested_material and resolve_conversion_material(requested_material) != material:
                warnings.append("Expliciete materiaalfallback genegeerd: betrouwbaar bronmateriaal heeft voorrang")
...
def profile_from_nc1(path: str | Path) -> ProfileDefinition:
    ...
    from cws_convertor.conversion_service import conversion_material_density

    weight = float(header.weight)
    if not math.isfinite(weight) or weight < 0.0:
        raise ValueError("Ongeldige NC1-profielmassa: niet-eindig of negatief")
    # kg/m ÷ kg/m³ gives m²; convert the result to mm². Missing density must
    # never imply steel when a material-dependent area is reconstructed.
    area = weight * 1_000_000.0 / conversion_material_density(header.material) if weight > 0.0 else 0.0
    profile = ProfileDefinition(
        ...
        properties={
            "mass_source": "NC1.header.weight",
            "mass_status": "source_declared" if weight > 0.0 else "unresolved",
            "source_material": str(header.material or ""),
        },
    )
    # A profile catalogue may supply a nominal steel mass in __post_init__.
    # It is not a replacement for absent source evidence on an imported part.
    profile.mass_kg_m = weight
    return profile
