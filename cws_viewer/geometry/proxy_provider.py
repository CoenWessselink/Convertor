...
PROVIDER_VERSION='cws-explicit-display-proxy-v1'

def _box(x,y,z):
    x=max(float(x),1.0);y=max(float(y),1.0);z=max(float(z),1.0)
    v=np.asarray([[0,0,0],[x,0,0],[x,y,0],[0,y,0],[0,0,z],[x,0,z],[x,y,z],[0,y,z]],dtype=np.float64)
    ...
    def load(self,request,settings,*,cancel_check:CancelCheck|None=None):
        ...
        if diam>0:
            ...
            if isinstance(bounds,dict):
                lo=bounds.get('minimum',[0,0,0]);hi=bounds.get('maximum',[0,0,0]);size=[max(0,float(hi[i])-float(lo[i])) for i in range(3)]
            if max(size)<=0:
                s=max(float(m.get('size_mm') or 0),10.);size=[max(length,s),s,s]
            v,t=_box(*size)
